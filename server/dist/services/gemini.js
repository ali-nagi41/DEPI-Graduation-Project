...
class GeminiService {
    ...
    async generateTasksFromGoal(projectName, goal, teamMembers) {
...
`;
    ...
    async generateCodeTemplate(taskTitle, description) {
        if (this.isMockMode || !this.genAI) {
            // Simulate delay
...
`;
    ...
    getMockTasks(projectName, goal, teamMembers) {
        const defaultTasks = [
            {
                id: (0, uuid_1.v4)(),
    ...
    getMockCodeTemplate(taskTitle, description) {
        let codeSnippet = '';
