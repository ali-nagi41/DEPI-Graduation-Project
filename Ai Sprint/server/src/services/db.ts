...
class DatabaseService {
  ...
  async getProjectById(id: string, userId: string): Promise<any | null> {
    if (!this.isFallback) {
      ...
      const projects = this.readLocalProjects();
      return projects.find(p => p._id === id && p.userId === userId) || null;
    }
  }

  async createProject(name: string, goal: string, tasks: Partial<ITask>[], userId: string): Promise<any> {
  ...
  async updateTask(projectId: string, taskId: string, updates: Partial<ITask>, userId: string): Promise<any | null> {
    if (!this.isFallback) {
      ...
      const projects = this.readLocalProjects();
      const project = projects.find(p => p._id === projectId && p.userId === userId);
      if (!project) return null;
